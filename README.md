...
### The Algorithm questions of DFS
#### Binary Tree Paths
```js
...
```
### The points:
- Traverse的停止条件为`root.left == null && root.right == null`, 而不是 root == null, 因为path早在parent node的时候就已经加过了； 还有一点需要注意的是path 不能先加再传入，因为这样加完了左边右边的path就被改变了，所以最终得不到正确的结果。必须把整个表达式传入，这样不会改变path在同一层的值。
- DivideAndConquer的方法中需要注意的是整个result都是局部变量， 不能设为全局变量，因为每一步这个数组都在增加成员，如果直接在result上面改变将会导致当树不规则的时候短值被覆盖导致无法加入。还有JavaScript的for...of不能在数组为非数字的条件下使用，map 也不行，所以只能用for...in，用key做链接
